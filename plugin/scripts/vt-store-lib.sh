...
vt_store_write_new() {
  ...
  # Required: STORE_PROJECT STORE_TITLE; optional STORE_TYPE STORE_WHY STORE_DEADLINE
  vt_store_ensure
  local id path ts type why due
  id=$(vt_store_next_id)
...
cleared_at=
ITEM
  vt_store_log_transition "$id" "-" "captured" "capture"
  printf '%s\n' "$id"
}
